...
> Получение истории генераций с фильтрацией и пагинацией
...
<CodeGroup>
  ```bash Все генерации theme={null}
  curl "https://polza.ai/api/v1/history/generations" \
    -H "Authorization: Bearer YOUR_API_KEY"
  ```

  ```bash С фильтрацией theme={null}
...
| `apiType`           | string       | Тип API (chat\_completions, responses, media и т.д.)  |
| `responseMode`      | string       | Режим ответа (sync, stream, async)                    |
| `status`            | string       | Статус генерации                                      |
| `finishReason`      | string       | Причина завершения (stop, length, tool\_calls и т.д.) |
